...
function cardRefreshTimes() {
  ...
  if (cardWidgetSchedule) {
    ...
    cardWidgetSchedule.querySelector("#p_span_year").innerHTML =
...
function cardTimes() {
  ...
  if (cardWidgetCalendar) {
    ...
    var rows =
      row_f == 0
        ? Math.floor((dates - row_h) / 7) + 1
        : Math.floor((dates - row_h) / 7) + 2;
    var calendar = cardWidgetCalendar.querySelector("#calendar-main");
    var gap = cardWidgetCalendar.querySelector("#calendar-date");
    switch (rows) {
      case 4:
        gap.style.fontSize = "36px";
        break;
      case 5:
        gap.style.fontSize = "48px";
        break;
      case 6:
        gap.style.fontSize = "64px";
        break;
      default:
        gap.style.fontSize = "64px";
    }
      ...
      for (let d = 0; d < 7; d++) {
        ...
        if (
          calendar.querySelector(
            ".calendar-r" + r + " .calendar-d" + d + " a"
          ) == null
        ) {
          calendar.querySelector(".calendar-r" + r).innerHTML +=
    ...
    cardWidgetCalendar.querySelector("#calendar-week").innerHTML =
      "第" + asideWeekNum + "周&nbsp;" + weekStr; //星期
    cardWidgetCalendar.querySelector("#calendar-date").innerHTML = date
      .toString()
      .padStart(2, "0"); //日期
    cardWidgetCalendar.querySelector("#calendar-solar").innerHTML =
